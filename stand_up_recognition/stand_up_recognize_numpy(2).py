...
class TimeUsedSumFT:
    def __init__(self):
        ...
        self.__sum = {
            ## "prefix": {"count": n, "total": sum}
        }
        self.begin = time.time()

    def update(self, prefix:str, tu:float):
        with self.__lock:
            t1 = time.time()
            if prefix not in self.__sum:
                self.__sum[prefix] = {"count": 0, "total": 0, "max": 0, "begin": t1 - tu - self.begin}
            self.__sum[prefix]["count"] += 1
            self.__sum[prefix]["total"] += tu
            self.__sum[prefix]["max"] = max(tu, self.__sum[prefix]["max"])
            self.__sum[prefix]["last"] = t1 - self.begin
...
class TimeUsedFT:
    def __init__(self, prefix:str, tus:TimeUsedSumFT=None, timeout=6.0):
        self.__prefix = prefix
        self.__timeout = timeout
        self.__tus = tus if tus is not None else TimeUsedSumFT()
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        d = time.time() - self.__t0
        self.__tus.update(self.__prefix, d)
...
class JudgeSuspiciousStandUp:
    ...
    def suspicious_stand_up_judge(id_df):
        ...
        if diff_y < 30 or len(id_df) < 8:
            return id_df_new
        kmeans = KMeans(n_clusters=2, random_state=42).fit(id_df_new[:,2].reshape(-1, 1))
        ...
        t0_act_judge_num = np.sum(t0_act_judge1) + np.sum(t0_act_judge2) + np.sum(t0_act_judge3)
        t0_act_judge_num2 = np.sum(t0_act_judge4)
        if t0_act_judge_num2 > 4:
            id_df_new[mask, -1]=1
        # only tolerate the time gap of 2 frames (at most 8s)
        elif gap > 30 and t0_act_judge_num < 3 and np.all(t0_series_diff < 13):
            id_df_new[mask, -1]=1
    ...
    def track(self,input_array_all,time_stamp_list):
        ...
        with TimeUsedFT("track for loop"):
            for time_idx, timestamp in enumerate(time_stamp_list):
                ...
                with TimeUsedFT("track for loop: update tracker"):
                    resultTracker, unmatched_dets = tracker.update(detections)

                with TimeUsedFT("track for loop: save initial detection results"):                    
                    # 2,  save the initial detection results and tracking results in dataframe
                ...
                3, find tracker id                                                                                                                                                                      
                """
                with TimeUsedFT("track for loop: last step"):
        ...
        with TimeUsedFT("track. combine different id ..."):
    ...
    def JudgeAndVisualize(self,person_series_array,head_series_array,video_path,source_file):

        stamp_list = np.unique(person_series_array[:,3])
        with TimeUsedFT("JudgeAndVisualize.first for loop"):
        ...
        with TimeUsedFT("JudgeAndVisualize.second for loop"):
...
if __name__ == '__main__':
    ...
    def test(opt):
        db = sq3.connect(opt.db)
        cur = db.cursor()
        cur.execute("select stamp,imgid,actid,x1,y1,x2,y2,cid,score from t_actions2 where imgid%2==0")
        rs = cur.fetchall()
        cur.close()
        db.close()

        J = JudgeSuspiciousStandUp(track_frame_num=opt.frame)
        result = {} # {stamp: output, ...}

        student_acts = np.array(rs, dtype=np.float32)   # N * (stamp,imgid,actid,x1,y1,x2,y2,cid,score)
        uni_stamp = np.unique(student_acts[:, 0])   # [0.0, 4.1, ... ]

        for stamp in uni_stamp:
            idx = np.where(student_acts[:, 0] == stamp)[0]
            acts = student_acts[idx]
            assert acts.ndim == 2 and acts.shape[1] == 9 and acts.shape[0] > 0
            with TimeUsedFT("main"):
                r = J.main(acts, stamp)
            result[stamp] = r

        logging.warning(f"time used: {json.dumps(TimeUsedSumFT().result(), indent=4)}")

        return result


    ap = ArgumentParser()
    ap.add_argument("db", help="sdb file name")
    ap.add_argument("--frame", default=15, type=int, help="number of frames per group")
    opt = ap.parse_args()
