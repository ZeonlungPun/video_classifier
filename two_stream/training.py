...
model=TwoStreamNetwork(num_classes=4).to(device)
...
for epoch in range(num_epochs):
    ...
    correct = 0
    total = 0
    for spatial_videos, temporal_videos, labels in train_loader:
        spatial_videos, temporal_videos, labels = spatial_videos.to(device),temporal_videos.to(device) ,labels.to(device)  # 將數據移動到GPU
        optimizer.zero_grad()
        outputs = model(spatial_videos, temporal_videos)
    ...
    total = 0
    with torch.no_grad():
        for spatial_videos, temporal_videos, labels in test_loader:
            spatial_videos, temporal_videos, labels = spatial_videos.to(device),temporal_videos.to(device) ,labels.to(device)
            outputs = model(spatial_videos, temporal_videos)
