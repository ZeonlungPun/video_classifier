...
class SIFTDescriptorDataset(Dataset):
    def __init__(self, root_dir,new_size,saved_keypoints,transform=None):
        ...
        self.labels = []
        self.class_map = {}
        self.saved_keypoints=saved_keypoints
        self.transform = transform
    ...
    def __getitem__(self, idx):
        ...
        # 在目標圖片上計算保存的關鍵點的描述子
        _, des = sift.compute(frame_cropped, self.saved_keypoints)
        if self.transform:
            des=self.transform(des)

        return des ,label


img=cv2.imread('/home/punzeonlung/fog_paper/input/batch_1/frame_at_00_00_40.jpg')
...
dataset = SIFTDescriptorDataset(root_dir,new_size=new_size,saved_keypoints=kp1 ,transform=transform)
