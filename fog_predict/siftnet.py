...
class OpticalFlowDataset(Dataset):
    ...
    def __getitem__(self, idx):
        while True:
            ...
            flow_path=os.path.join(self.flow_root_dir+'/'+list(self.class_map.keys())[label]+'/'+flow_path)
            ...
            if len(sub_flow_path_list) < num_frame:
            ...
            for sub_flow_path in sub_flow_path_list:
                final_path_=os.path.join(flow_path,sub_flow_path)
                flow_=np.expand_dims(np.load(final_path_),2)
                flow.append(flow_)

            flow = np.concatenate(flow, axis=-1)
            #random_frame = random.randint(0, flow.shape[2] - 1)
            random_frame = np.random.permutation(flow.shape[2])[:10]
            flow = flow[:, :, random_frame]
            ...
            return flow, label ,torch.tensor(float(vis_num.iloc[0]),dtype=torch.float)
